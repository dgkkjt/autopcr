...
from ...model.common import ChangeRarityUnit, ExtraEquipChangeSlot, ExtraEquipChangeUnit, GrowthParameterList, InventoryInfoPost, SkillLevelInfo, SkillLevelUpDetail, UnitData
...
class UnitController(Module):
    ...
    async def unit_skill_up(self, location: eSkillLocationCategory, target_level: int, current_level: int, free: bool):
        ...
        if free:
            self._log(f"{self.unit_name}技能{self.skill_name[location]}免费升至{current_level + step}级")
    ...
    async def unit_skill_up_aware(self, location: eSkillLocationCategory, skill: Callable[[], SkillLevelInfo], target_skill_level: int):
        ...
        if limit and limit.skill_level < target_skill_level and skill().skill_level < limit.skill_level:
            ...
            await self.unit_skill_up_aware(location, skill, limit.skill_level)
        ...
        if target_skill_level > self.unit.unit_level:
            ...
            await self.unit_level_up_aware(target_skill_level)
    ...
    async def unit_promotion_up(self, target_promotion_level: int, free: bool):
        if free:
            self._log(f"{self.unit_name}免费升至品级{target_promotion_level}")
            ...
            if bad:
                bad_list = ','.join([f"{db.get_inventory_name_san(item)}缺少{cnt}片" for item, cnt in bad.items()])
                raise AbortError(f"{self.unit_name}升至品级{target_promotion_level}所需材料不足: {bad_list}")
    ...
    async def unit_promotion_up_aware(self, target_promotion_level: int):
        limit = await self.is_growth_unit()
    ...
    async def unit_level_up(self, target_level: int, free: bool):
        if free:
            self._log(f"{self.unit_name}免费升至{target_level}级")
    ...
    async def unit_level_up_aware(self, target_level: int):
        limit = await self.is_growth_unit()
        ...
        if limit and target_level > limit.unit_level and self.unit.unit_level < limit.unit_level:
                ...
                await self.unit_level_up_aware(limit.unit_level)
    ...
    async def unit_equip_slot(self, equip_id: int, slot_num: int, free: bool):
        if free:
            self._log(f"{self.unit_name}免费装备{db.get_equip_name(equip_id)}")
            ...
            if bad:
                bad_list = ','.join([f"{db.get_inventory_name_san(item)}缺少{cnt}片" for item, cnt in bad.items()])
                raise AbortError(f"{self.unit_name}{slot_num}位装备{db.get_equip_name(equip_id)}所需材料不足: {bad_list}")
    ...
    async def unit_equip_slot_aware(self, equip_id: int, slot_num: int):
        limit = await self.is_growth_unit()
        if db.equip_data[equip_id].require_level > self.unit.unit_level:
            ...
            await self.unit_level_up_aware(db.equip_data[equip_id].require_level)

        free = False
        if limit is not None:
            if limit.promotion_level > self.unit.promotion_level:
                free = True
            elif limit.promotion_level == self.unit.promotion_level:
                raw_pos = [6, 4, 2, 5, 3, 1]
                equip_pos = sorted(raw_pos, key=lambda x: (-self.unit.equip_slot[x - 1].is_slot, raw_pos.index(x)))
                for i in range(6):
                    rp = raw_pos[i]
        ...
        await self.unit_equip_slot(equip_id, slot_num, free)
    ...
    async def unit_equip_enhance_aware(self, slot_num: int, star: int):
        equip = self.unit.equip_slot[slot_num - 1]
        if not equip.is_slot:
            self._log(f"{self.unit_name}{slot_num}位装备未装备{db.get_equip_name(equip.id)}，先装备")
            await self.unit_equip_slot_aware(equip.id, slot_num)

        limit = await self.is_growth_unit()
        promotion_limit = db.get_equip_max_star(equip.id)
        if star > promotion_limit:
            self._log(f"{slot_num}号位装备预设{star}星级超过了最大星级{promotion_limit}，提升至最大星级{promotion_limit}")
            star = promotion_limit

        if self.unit.equip_slot[slot_num - 1].enhancement_level < star:
            free = False
            if limit is not None:
                if limit.promotion_level > self.unit.promotion_level:
                    free = True
            ...
            await self.unit_equip_enhance(equip.id, slot_num, star, free)

    async def unit_unique1_slot_aware(self):
        ...
        if self.unit.promotion_level < 7:
            ...
            await self.unit_promotion_up_aware(7)
    ...
    async def unit_unique1_slot(self, free: bool):
        if free: # impossible
            ...
            if self.client.data.get_inventory((eInventoryType.Equip, equip_id)) == 0:
                ...
                if bad:
                    bad_list = ' '.join([f"{db.get_inventory_name_san(item)}缺少{cnt}片" for item, cnt in bad.items()])
                    raise AbortError(f"制作{self.unit_name}专武1所需材料不足: {bad_list}")
    ...
    async def unit_unique_equip1_rank_up_aware(self, target_rank: int):
        limit = await self.is_unique_growth_unit()
        ...
        if require_level > self.unit.unit_level:
            ...
            if not self.auto_level_up:
                raise AbortError(f"当前设置不自动拉角色等级，无法升级")
            await self.unit_level_up_aware(require_level)
        ...
        if bad:
            bad_list = ','.join([f"{db.get_inventory_name_san(item)}缺少{cnt}片" for item, cnt in bad.items()])
            raise AbortError(f"{self.unit_name}专武1突破至{target_rank}品级所需材料不足: {bad_list}")
        ...
        if self.unit.unique_equip_slot[0].is_slot == 0:
            ...
            await self.unit_unique1_slot_aware()
    ...
    async def unit_unique_equip2_enhance_aware(self, target_level: int):
        ...
        if limit and target_pt > limit.unique_equip_strength_point_2 and self.unit.unique_equip_slot[1].enhancement_pt < limit.unique_equip_strength_point_2:
            ...
            await self.unit_unique_equip2_enhance_aware(level)
    ...
    async def unit_unique_equip1_enhance_aware(self, target_level: int, limit: Union[GrowthParameterUnique, None] = None):
        ...
        if target_rank > self.unit.unique_equip_slot[0].rank:
            ...
            await self.unit_unique_equip1_rank_up_aware(target_rank)
    ...
    async def promote(self, target_level: int = 1, target_star: int = 1, target_dear: int = 1, target_promote_rank: int = 1, target_equip_star: List[int] = [-1, -1, -1, -1, -1, -1], target_skill_ub_level: int = 1, target_skill_s1_level: int = 1, target_skill_s2_level: int = 1, target_skill_ex_level: int = 1, target_unique1_level: int = 0, target_unique2_level: int = -1, target_ex1_id: int = -1, target_ex1_star: int = 0, target_ex2_id: int = -1, target_ex2_star: int = 0, target_ex3_id: int = -1, target_ex3_star: int = 0, cb_ex: bool = False):
        try:
            ...
            if self.unit.unit_level < target_level:
                await self.unit_level_up_aware(target_level)
        ...
        try:
            if self.unit.promotion_level < target_promote_rank:
                await self.unit_promotion_up_aware(target_promote_rank)
            ...
            if self.unit.promotion_level == target_promote_rank:
                for i in [5, 3, 1, 4, 2, 0]:
                    ...
                    if star != -1:
                        if not equip.is_slot:
                            await self.unit_equip_slot_aware(equip.id, i + 1)
                        ...
                        if equip.enhancement_level < star:
                            await self.unit_equip_enhance_aware(i + 1, star)
        ...
        try:
            if len(self.unit.unique_equip_slot) > 1 and (not self.unit.unique_equip_slot[1].is_slot and target_unique2_level != -1 
                ...
                await self.unit_unique_equip2_enhance_aware(target_unique2_level)
        except Exception as e:
        ...
        try:
            ...
            if self.unit.union_burst and self.unit.union_burst[0].skill_level < target_skill_ub_level:
                await self.unit_skill_up_aware(eSkillLocationCategory.UNION_BURST_SKILL, lambda: self.unit.union_burst[0], target_skill_ub_level)

            if self.unit.main_skill and self.unit.main_skill[0].skill_level < target_skill_s1_level:
                await self.unit_skill_up_aware(eSkillLocationCategory.MAIN_SKILL_1, lambda: self.unit.main_skill[0], target_skill_s1_level)

            if len(self.unit.main_skill) > 1 and self.unit.main_skill[1].skill_level < target_skill_s2_level:
                await self.unit_skill_up_aware(eSkillLocationCategory.MAIN_SKILL_2, lambda: self.unit.main_skill[1], target_skill_s2_level)

            if self.unit.ex_skill and self.unit.ex_skill[0].skill_level < target_skill_ex_level:
                await self.unit_skill_up_aware(eSkillLocationCategory.EX_SKILL_1, lambda: self.unit.ex_skill[0], target_skill_ex_level)
        ...
        while not stop:
            for id in equip_slot_list: 
                ...
                if equip.enhancement_level < db.get_equip_max_star(equip.id):
                    await self.unit_equip_enhance_aware(slot_num, equip.enhancement_level + 1)
                    stop = True
                    break
            else:
                self._log(f"所有装备均已满强化，需提升角色品级")
                await self.unit_promotion_up_aware(self.unit.promotion_level + 1)
            ...
            for pos, skill in skills:
                try:
                    if skill().skill_level < self.unit.unit_level:
                        await self.unit_skill_up_aware(pos, skill, skill().skill_level + 1)
                        stop = True
                        break
...
@inttype("unit_exceed_mana_keep", "保留Mana下限（亿）", 10, list(range(1000)))
@default(False)
...
@singlechoice("unit_evolution_to_rarity", "目标星级", 5, list(range(2, 6)))
...
class set_my_party2(SetMyParty):
    ...
    async def is_to_max(self) -> bool:
        return self.get_config('set_my_party2_to_max')
