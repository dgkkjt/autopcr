...
@description('刷分用。若已进入黎明界，会先检查当前公会/难度与开局是否符合要求，不符合时撤退后重刷。完美开局指一层双角色、二层双角色+遗物+EX+遗物/商店，三层角色+遗物+遗物/事件，四层双角色+双EX，五层遗物/事件+双EX。遗物固定分，事件分可高可低。开启完美开局时会忽略“区域1双角色必须连通”选项。')
@name('黎明界刷开局')
...
class labyrinth_start_reroll(Module):
    ...
    def _area1_double_character_connected(self, route: List) -> bool:
        required_columns = {
            column
            for column, block_types in self.AREA_REQUIREMENTS[1].items()
            if eLabyrinthBlockType.TICKET in block_types
        }
        route_columns = {
            block.column
            for block in route
            if self._block_type(block) == eLabyrinthBlockType.TICKET and block.column in required_columns
        }
        return required_columns.issubset(route_columns)

    def _find_area_route(self, area: int, map_list: List, area3_bosses: Set[int], area5_bosses: Set[int], perfect_start: bool, expected_block_types: Dict[int, Dict[int, Set[eLabyrinthBlockType]]], require_area1_double_character_connected: bool) -> Tuple[Optional[List], str]:
    ...
    def _find_routes(self, map_list: List, difficulty: int, area3_bosses: Set[int], area5_bosses: Set[int], perfect_start: bool, expected_block_types: Dict[int, Dict[int, Set[eLabyrinthBlockType]]], require_area1_double_character_connected: bool) -> Tuple[Optional[Dict[int, List]], str]:
        routes: Dict[int, List] = {}
        failures = []
        for area in self._target_areas(difficulty):
            route, reason = self._find_area_route(area, map_list, area3_bosses, area5_bosses, perfect_start, expected_block_types, require_area1_double_character_connected)
            if not route:
    ...
    async def do_task(self, client: pcrclient):
        ...
        if top.enter_id:
            ...
            if same_guild and same_difficulty:
                ...
                routes, reason = self._find_routes(resume.map_list or [], difficulty, area3_bosses, area5_bosses, perfect_start, expected_block_types, require_area1_double_character_connected)
                if routes:
        ...
        for attempt in range(1, max_count + 1):
            ...
            routes, reason = self._find_routes(enter.map_list or [], difficulty, area3_bosses, area5_bosses, perfect_start, expected_block_types, require_area1_double_character_connected)
            if routes:
